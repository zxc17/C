...
int findin(int deep, char *fileName)
{
	...
	while (nRet >= 0)
	{
		...
		else
		{
			...
			switch (copyType)
			{
			case '0': goto switchCom;
			case '1': sprintf_s(cmd, "copy \"%s\" \"%s\\%s%s\"", data.name, dstPath, fileName, data.name); break;
			case '2': sprintf_s(cmd, "copy \"%s\" \"%s\\%s\"", data.name, dstPath, data.name); break;
				////char tmp[MAX_PATH];
				//for (int jk = 1; jk <= 99; jk++)
				//{
				//	if (!_access(data.name, 0))	//重复文件名?
				//	{
...
int main()
{
	...
	fflush(stdin);
	do{
		printf("输入操作类型：([0].仅遍历 1.复制到一个文件夹 2.复制到一个文件夹并添加路径前缀): ");
		copyType = getchar();
		fflush(stdin);
		if (copyType == '\n') copyType = '0';
	} while (copyType<'0' || copyType>'2');
